...
1. Helm Deployments
    ...
    3. You need to replace/fill in values for two deployments:
        1. `aws-system/aws-auth.yaml`
            1. If you are creating a single node type you can delete the `aws-system/aws-auth.yaml` file and proceed otherwise replace the values for the two role ARNs with the ones from the pulumi output. If you need to get the output again you can run `pulumi stack output` and it will print the values. The first value is for the Linux ARN and the second value is for the Windows ARN.
